...
As seen above, we found that our dataset is clean and has no missing values. Now, we can move on to insepcting the features within the dataset. 

By viewing our dataset printed on the second page, we see that we have a column \texttt{Functioning.Day} which indicates whether bikes were available for rent during that day or not. Since our goal is to predict the hourly count of rented bikes, we only need the data of operational days since the count is simply 0 on non-operational days. Therefore, we will remove observations made during non-functioning days, then drop the \texttt{Functioning.Day} column since we have no more use of it anymore. 
...
We then looked at a summary of each feature within the dataset to further unerstand the features we are dealing with and determine the next step forward.
...
Next, instead of using the date of each observation as a predictor, we simply need to know whether that day is a weekend or not. This is because we would expect bike rental patterns to differ between weekdays and weekends. To achieve this, I will create an \texttt{Is.Weekend} column. 

```{r transforming date col}
bike_df4$Date <- as.Date(bike_df4$Date, format = "%d/%m/%Y")
bike_df4$Is.Weekend <- as.integer(weekdays(bike_df4$Date) %in% c("Saturday", "Sunday"))
bike_df4 <- bike_df4 %>% 
  select(-Date)
```

We now have an \texttt{Is.Weekend} column that has the value 1 if that observation was recorded on a weekend and 0 otherwise. Notice that we also removed the \texttt{Date} column since we do not need it anymore.
...
Using the above formulas, we will proceed by calculating the above 3 indexes, then add them to the dataset as new variables: \texttt{HeatIdx}, \texttt{WindChill} and \texttt{HumiDex}. 
```{r creating new features}
bike_df4$HeatIdx <- 0.5 * ((bike_df4$Temperature * (9/5) + 32) + 61 + (((bike_df4$Temperature * (9/5)
                   + 32)-68)*1.2) + (bike_df4$Humidity*0.094))
...
bike_df4$HumiDex <- bike_df4$Temperature + 0.5555*(6.11 * exp(5417.7530*(1/273.15 - 
...
```{r dist of response, fig.width=6, fig.height=4}
...
```{r square root transformation, fig.width=6, fig.height=4}
bike_df4$SqrRBC <- sqrt(bike_df4$Rented.Bike.Count)
ggplot(bike_df4, aes(x = SqrRBC)) +
  geom_histogram(binwidth = 5, fill = "purple", color = "black") +
  labs(title = "Histogram of Square Root Transformed Rented Bike Count",
...
Consequently, we removed `r dim2[1]-dim[1]` outliers from the dataset.
...
We will proceed by performing **one-hot encoding** on both these columns and the columns \texttt{Rained} and \texttt{Snowed}. 
...
head(bike_df7,2)
```
Next, we will convert the \texttt{Hour} column from an integer to a factor, and remove the \texttt{mean} and \texttt{sd} columns from the dataset.
...
## Numerical Variables
Next, we will examine our numerical variables and visualize each of them against \texttt{SqrRBC}:
...
From the plots above, we see that our square root transformed rented bike count has a small peak at 8-9am and is the highest at 6-7pm which coincides with times the people go to and leave work respectively. Next, \texttt{Temperature}, \texttt{Dew.Point.Temperature}, \texttt{HeatIndex}, \texttt{WindChill} and \texttt{HumiDex} all seem to have some positive correlation with \texttt{SqrRBC}.
...
```{r heatmap, fig.width=6, fig.height=6, echo=FALSE}
...
```
From the heatmap, it can evidently be seen that the features \texttt{Temperature}, \texttt{Dew.Point.Temperature}, \texttt{HeatIndex}, \texttt{WindChill} and \texttt{HumiDex} are highly correlated with each other. This is to be expected since the latter 3 features were all computed using \texttt{Temperature}, with one of them also computed using \texttt{Dew.Point.Temperature}. Thus, those two variables should be removed. Although \texttt{HeatIndex}, \texttt{WindChill} and \texttt{HumiDex} are still highly correlated, they may improve model performance, which is why we did not remove these predictors. Instead, we will also remove \texttt{Wind.Speed} and \texttt{Humidity} since those features were also were to calculate \texttt{WindChill} and \texttt{HeatIdx} respectively. \

Next, to remove dependancy within the dataset, we also want to remove \texttt{SeasonsWinter}, \texttt{HolidayNo Holiday}, \texttt{RainedYes} and \texttt{SnowedYes} since one one-hot encoded column for each variable must be removed.
...
Next, we will also remove \texttt{Wind.Speed} and \texttt{Humidity} since \texttt{WindChill} and \texttt{HeatIndex} are dependant on them respectively.\

Finally, we will remove one column from each one-hot encoded categorical variable: \texttt{SeasonsWinter} and \texttt{HolidayNo Holiday} to avoid multicollinearity. Our remaining features are as follows: \
...
The summary of our MLR model boasts a multiple R-squared of 0.769, indicating that 76.9% of the variability in \texttt{SqrRBC} is explained by the model, which suggests a strong fit. Additionally, most predictors are statistically significant in predicting \texttt{SqrRBC}, with p-values less than 0.05. However, \texttt{Snowed} is an exception, with a p-value of 0.397656 which is a lor larger than 0.05. This implies that the presence of snow might not significantly influence bike rental decisions of consumer. To determine whether we should remove \texttt{Snowed} from the model, we will perform an ANOVA test.

```{r}
lm2 <- lm(SqrRBC ~ Hour + Visibility + Solar.Radiation + Rained + Is.Weekend + HeatIdx + 
            WindChill + HumiDex + SeasonsAutumn + SeasonsSpring + SeasonsSummer + 
            HolidayHoliday, data=train_data)
summary(lm2)
anova(lm1, lm2)
```
...
Now that we have obtained our model, let us evaluate its predictive power by calculating its mean squared error (MSE). Note that we use MSE here instead of root mean squared error (RMSE) since MSE will have the same units as \texttt{Rented.Bike.Count} which facilitates interpretation.  
...
We found that our model has an MSE of `r round(mse,4)`, which seems reasonable given that rented bike count in our dataset ranges from 2 to 3,556, with a median of 542. However, to further improve our model, we will examine its residual plots to check for behaviour that could indicate areas where the model may not be performing well.
...
```{r residual plots 2, fig.width=6, fig.height=4}
plot(lm2, which = 1)
```
...
## Spline regression
Refering back to the pair plots of each of the numerical predictors against \texttt{SqrRBC}, we notice that \texttt{Solar.Radiation} and \texttt{Visibility} exhibit non-linear relationships with \texttt{SqrRBC}. Therefore, we will incorporate splines for these predictors. We will also add splines on \texttt{HeatIdx}, \texttt{WindChill} and \texttt{HumiDex} because, while they show some linear relationship with \texttt{SqrRBC}, it is not particularly strong.
...
From the summary of our natural spline model, we can observe that with the addition of natural splines, our multiple R-squared value increased from 0.769 to 0.815, which is a `r round(((0.815-0.769)/0.769)*100,2)`\% increase.  
...
```{r spline residual, fig.width=6, fig.height=4, echo=FALSE}
...
```{r predictor importance ranking}
kable(importance(rf_model))
```

From the table above, we can surmise that the predictors \texttt{Snowed} and \texttt{HolidayHoliday} have low importance in the decision-making process of the random forest model. While \texttt{SeasonsSpring} and \texttt{SeasonsSummer} also show lower importance, we decided to keep them because \texttt{SeasonsAutumn} has a high importance score, and all three only require a single input from the client (type of season). Therefore, we will proceed by removing \texttt{Snowed} and \texttt{HolidayHoliday} from our dataset and retrain the random forest model.
...
After dropping the two predictors, we notice a slight increase in the MSE which increased from `r round(mse3, 4)` to `r round(mse4, 4)`. Despite this minor loss in predictive accuracy, the trade-off is beneficial as it simplifies the model and enhances its usability and interpretability for clients.
...
```{r tune mtry, fig.width=6, fig.height=4}
set.seed(435)
...
```{r 10-fold cross validation results, fig.width=6, fig.height=4}
mean_cv_error <- mean(cv_errors)

# Plot the cross-validation errors
plot(cv_errors, type = "b", pch = 19, col = "purple",
     xlab = "Fold", ylab = "Error", main = "10-fold Cross-Validation Mean Squared Error")
```

Since the mean of the MSE is `r round(mean_cv_error, 2)` which is similar to the prior attained mean squared error of `r round(mse5, 2)`, we can conclude that our random forest model is robust.
...
predict_rbc <- function(hr, temp, hum, ws, visibility, dpt, sr, rain, weektype, season) {
  ...
  data_point <- data.frame(
    ...
    HumiDex = temp + 0.5555*(6.11 * exp(5417.7530*(1/273.15 - 1/(273.15 + dpt)))-10),
